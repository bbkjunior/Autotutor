#ЧТОБЫ ИЗБЕЖАТЬ ПРОБЛЕМ НАДО ПОЛЬЗОВАТЬСЯ ТОЛЬКО UDPIPE БЕЗ МАЙСТЕМА
#ПОКА ЧТО ПРИ НАРУШЕНИИ АССЕРТА УБИВАЕМ ПРОЦЕСС
import copy 
...
def lemmatize_text_from_grammar_map(text_gr_map):
    ...
    for sentence_gr_map in text_gr_map:
        ...
        for word_grammar_features in sentence_gr_map:
            ...
            #print(word)
            check_letters = re.compile('[a-zA-Z]+')
            #print(word_grammar_features)
            if(word.isdigit()):
                #print("digir")
                lemma = word
                sentence += lemma + ' '
                ...
                #pass
            elif(word_grammar_features['analysis'] ==[] and not check_letters.match(word)):
                pass
            else:
                if(len(word_grammar_features['analysis']) > 0):
...
def get_weights_empty_list(clean_lemm_sentences_list, clean_orig_sentences_list):
    ...
    for lemm_sentence, sentence in zip(clean_lemm_sentences_list,clean_orig_sentences_list):
        ...
        #print(sentence)
        sentence_weights = []
        if len(lemm_sentence.split()) != len(sentence.split()):
            print(lemm_sentence.split())
...
def apply_udpipe_properties(ud_parsed_file, weights_input):
    ...
    for sent_w in weights_sentences:
        for word_w in sent_w:
            ...
            ud_word = ud_properties[ud_word_index][1].lower()
            clean_ud_word = ''
            for char in ud_word:
                if char != " " and char not in full_punctuation:
                    clean_ud_word += char.lower()
            
            if(word_w['orig_word'] != clean_ud_word):
                print("WORD MISLAIGNMENT DETECTED")
                print("from wight", word_w['orig_word'])
                print("from ud parser", clean_ud_word)
            assert (word_w['orig_word'] == clean_ud_word), "Oh no! This assertion failed!"
            #print(type(word_w))
...
def update_pos(weights_list, gr_map):
    ...
    if(len(sentences_weights_list_output) != len(gr_map)):
        print("DIFFERENT NUMBER OF SENTENCES DETETCETD")
    ...
    for weights_sentence, gr_map_sentence in zip(sentences_weights_list_output, gr_map):
        if(len(weights_sentence) != len(gr_map_sentence)):
            print("DIFFERENT NUMBER OF WORDS DETETCETD")
            print(weights_sentence)
            print(gr_map_sentence)
...
def calculate_text_map(file,udpipe_file):
	...
	try:
		...
		print("get_weights_empty_list went wrong")
		return None
		
	try:
		text_weighs_synt = apply_udpipe_properties(udpipe_file, empty_weights_1) 
	except:
		print("apply_udpipe_properties went wrong")
		return None
