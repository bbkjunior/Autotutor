...
def get_recommended_text_json(answers_dict_json, raw_json = False, save_json_to_directory = False):
    ...
    for rec_text_el in sorted_text_feat_dict[:5]:
        ...
        raw_text = db_3000.iloc[text_ind]['text']
        # if len (raw_text) > 50:
        # text_name = "recommended_text_" + str(rex_text_index)
        rec_texts_list.append({"text_ind":text_ind,"raw_text": raw_text})
    
    negative_sorted_text_feat_dict = sorted(text_recommendation_dict.items(), key=operator.itemgetter(1), reverse = True)
    non_rec_texts_list = []
    for non_rec_text_el in negative_sorted_text_feat_dict[:5]:
        text_ind = int(non_rec_text_el[0])
        raw_text = db_3000.iloc[text_ind]['text']
        # if len (raw_text) > 50:
        # text_name = "recommended_text_" + str(rex_text_index)
        non_rec_texts_list.append({"text_ind":text_ind,"raw_text": raw_text})
